...
module.exports = util = {
	...
	matchers: {
		...
		pug: [
			// script with quoted src
			{ pattern: /(script.*\(?src=["'])([\s\S]+?)(["']\s*\))/gi, fallback: true },

			// link with quoted href and optional end tag
			{ pattern: /(link.*\(?href=["'])([\s\S]+?)(["'](?:\s*\))?)/gi, fallback: true },

			// img with quoted src
			{ pattern: /(img.*\(?src=["'])(.+?)(["'][^\)]*\))/gi, fallback: false },
	...
	getVersionInfo: function (fileInfo, opts) {
		...
		if (!opts._versionInfoCache) {
			opts._versionInfoCache = {};
		}
